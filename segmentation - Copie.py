...
#----------------------------------------------------------------------------------------------------
def get_boxes_contour(splits_1) :
    boxes = list()
    for s in range(len(splits_1)) :
        split = splits_1[s]
        ...
        for box in detect_contour(split) :
...
def get_boxes_model(splits_path) :
    boxes_model = list()
    for s in range(len(splits_path)) :
        options = define_options(splits_path[s])
        row, col = int(s / 4), int(s % 4)  
        for box in detect_model(options, print_msg = False)  :
            x1, y1, x2, y2 = box[0], box[1], box[2], box[3]
            w, h = x2 - x1, y2 - y1
            x1 += col * window_size - margin
            y1 += row * window_size - margin
            boxes_model.append([int(x1), int(y1), int(x1) + w, int(y1) + h])
    return boxes_model
#####################################################################################################
# EXECUTION                                    
#####################################################################################################
...
start_time_step = time.time()
print_img(splits[0], "split 1")
splits_1 = copy.deepcopy(splits)
boxes = get_boxes_contour(splits_1)
print_img(splits[0], "split 2")
...
# print("\nMERGE SPLITTED IMAGES :")
# start_time_step = time.time()
# merged = merge_image(splits_1, margin)
# end_time_step = time.time()
# print("     ", "TEMPS EXECUTION : %s secondes" % (end_time_step - start_time_step))
#-----Draw boxes
print("\nDRAW BOXES :")
...
splits_path = save_splits(splits)
print("     ", "TEMPS EXECUTION : %s secondes" % (end_time_step - start_time_step))
# pprint(splits_path)
#-----Segmentation of red blood cells
start_time_step = time.time()
boxes = get_boxes_model(splits_path)
print("\n\nSEGMENTATION OF RED BLOOD CELLS :")
print("     ", "Number of boxes after NMS :", len(boxes))
end_time_step = time.time()
print("     ", "TEMPS EXECUTION : %s secondes" % (end_time_step - start_time_step))
...
print("\nMERGE SPLITTED IMAGES :")
start_time_step = time.time()
merged = merge_image(splits_2, margin)
end_time_step = time.time()
print("     ", "TEMPS EXECUTION : %s secondes" % (end_time_step - start_time_step))
#-----Draw boxes
print("\nDRAW BOXES :")
...
print_img(img, "Segmented by Model")
end_time_step = time.time()
print("     ", "TEMPS EXECUTION : %s secondes" % (end_time_step - start_time_step))
#-----End Timer
end_time = time.time()
